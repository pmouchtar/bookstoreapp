...
class UserProfileControllerTest {
    ...
    void getUserProfile_success() throws Exception {
        ...
        mockMvc.perform(get(BASE_URL).header(HEADER, USER_ID.toString())).andExpect(status().isOk())
                .andExpect(jsonPath("$.username").value("jdoe")).andExpect(jsonPath("$.firstName").value("John"));
    }

    @Test
    void getUserProfile_notFound_returns404() throws Exception {
        Mockito.when(userService.findUserById(USER_ID)).thenThrow(new ResourceNotFoundException("User not found"));

        mockMvc.perform(get(BASE_URL).header(HEADER, USER_ID.toString())).andExpect(status().isNotFound())
    ...
    void updateUserProfile_success() throws Exception {
        ...
        Mockito.when(userService.updateUserProfile(eq(USER_ID), any())).thenReturn(dummyEntity);

        mockMvc.perform(put(BASE_URL).header(HEADER, USER_ID.toString()).contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(req))).andExpect(status().isOk())
                .andExpect(jsonPath("$.firstName").value("Jane")).andExpect(jsonPath("$.username").value("janed"));
    }
    ...
    void deleteUserProfile_success() throws Exception {
        Mockito.doReturn(true).when(userService).deleteUserById(USER_ID);
        mockMvc.perform(delete(BASE_URL).header(HEADER, USER_ID.toString())).andExpect(status().isNoContent());

    }

    @Test
    void deleteUserProfile_notFound_returns404() throws Exception {
        Mockito.doThrow(new ResourceNotFoundException("User not found")).when(userService).deleteUserById(USER_ID);

        mockMvc.perform(delete(BASE_URL).header(HEADER, USER_ID.toString())).andExpect(status().isNotFound())
                .andExpect(jsonPath("$.message").value(containsString("User not found")));
    }
}
