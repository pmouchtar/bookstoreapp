...
public record RegistrationRequestDto(@NotBlank(message = "Username is required") //
@Size(min = 4, max = 30, message = "Username must be between 4 and 30 characters") //
@Pattern(regexp = "^(?!\\d+$)[A-Za-z\\d._-]+$", //
        message = "Username cannot be only numbers and may contain"//
...
@Pattern(regexp = "^(?=.*[A-Z])(?=.*[a-z])(?=.*\\d).*$", //
        message = "Password must contain at least one uppercase letter,"//
                + " one lowercase letter, and one number") String password, //
        @NotBlank(message = "First name is required") //
        @Size(max = 50, message = "First name must be at most 50 characters") //
        @Pattern(regexp = "^[A-Za-zΑ-Ωα-ωΆ-Ώά-ώ]+$", //
